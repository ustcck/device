...
export default class ThresholdComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('threshold-heading'));
...
export class ThresholdDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('deviceApp.threshold.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-threshold'));
}
