...
export default class PatrolDeviceComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('patrol-device-heading'));
...
export class PatrolDeviceDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('deviceApp.patrolDevice.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-patrolDevice'));
}
